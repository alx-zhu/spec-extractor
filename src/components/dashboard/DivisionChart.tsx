...
import type { DivisionStat } from "@/hooks/useDashboardAnalytics";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg px-3 py-2 shadow-md text-sm">
      ...
      <p className="text-gray-500">
        {payload[0].value} {payload[0].value === 1 ? "product" : "products"}
      </p>
    </div>
  );
}
...
export function DivisionChart({ data }: DivisionChartProps) {
  ...
  const chartData = data.map((d) => ({
    ...d,
    label: `${d.code} – ${truncate(d.name, TRUNCATE_LENGTH)}`,
  }));
